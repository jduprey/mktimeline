...
def test_read_event():
    ev = Event("./test-data/test-events/events/1876--_west_publishing_company/1876--_west_publishing_company.md")
    print(ev.event_data)
    assert ev.event_data["text"]["headline"] == "West Publishing Company"
    assert "The nation’s most innovative and most successful" in ev.event_data["text"]["text"]
    assert "media" in ev.event_data, "Event should have media"


def test_write_event():
    # read in an event, orig_ev
    orig_ev = Event("./test-data/test-events/events/1876--_west_publishing_company/1876--_west_publishing_company.md")
